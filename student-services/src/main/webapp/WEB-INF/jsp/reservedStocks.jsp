...
<body class="bg-light">
    <!-- Create navigation for the website within a <header> tag -->
     ...
     <h1>Reserved Stocks</h1>
     	<c:choose>
     	<c:when test="${!userComp.getResStocks().isEmpty()}">
          ...
          <div class="row font-weight-bold mt-4 text-center border-bottom">
           ...
           <c:forEach var="compStocks" items="${resStocks}">
          	<div class="row mt-3 border border-primary">
          		...
          		<div class="col remove-padding">
          			...
          			<div class="row mb-2">
          				<div class="col remove-padding">
          					<p class="border-primary border-bottom border-top">Buy a Reserved Stock</p>
          						<!-- <form action="https://devep2.datamax.bg/ep2/epay2_demo/" method=post>
 									 <input type=hidden name=PAGE value="paylogin">
 									 <input type=hidden name=MIN value="${traderid}">
									 <input type=hidden name=INVOICE value="${100000000 + rng.nextInt(900000000)}">
									 <input type=hidden name=TOTAL value="${Math.floor(compStocks.getPricePerItem() * compStocks.getQuantity())}"> 
 									<input class="btn btn-primary" type=submit value="Buy Stock">
 								</form> -->
 								<form action="https://demo.epay.bg/" method="post">
    								<input type="hidden" name="PAGE" value="paylogin">
    								<input type="hidden" name="ENCODED" value="${compStocks.getBase64Encoded()}">
    								<input type="hidden" name="CHECKSUM" value="${compStocks.getSha1HMAC()}">								
          ...
          <c:when test="${userComp.getResStocks().isEmpty()}">
          	<div>
          		<p class="text-danger">No reserved stocks by company!</p>
          		<a href="/profile" class="btn btn-danger">Go back to Profile Page</a>
          	</div>
          </c:when>
          </c:choose>
    </main>
