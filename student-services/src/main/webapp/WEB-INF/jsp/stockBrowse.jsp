...
<body class="bg-light">
    <!-- Create navigation for the website within a <header> tag -->
     ...
     <main class="container text-center">
      <h4>Filter stock offers by Company Name</h4>
    	 <form method="POST" action="/stockBrowse">
                    <select class="form-control"  name="compName" class="mt-2">
                    	<c:forEach var="compData" items="${companyList}">
                    	<c:choose>
                    		<c:when test="${param.compName == compData.companyName}">
                    	  		<option value="${compData.companyName}" selected>${compData.companyName}</option>
                    	  	</c:when>
                    	  	<c:otherwise>
                    	  		<option value="${compData.companyName}">${compData.companyName}</option>
                    	  	</c:otherwise>
                    	</c:choose>
                    	</c:forEach>
                    </select>
                    <br>
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                    <input class="btn btn-primary mt-3" name="searchCompany" type="submit" value="Submit"/>
          </form>  
          <div class="container mt-2">
          <c:if test="${not empty param.searchCompany}">
