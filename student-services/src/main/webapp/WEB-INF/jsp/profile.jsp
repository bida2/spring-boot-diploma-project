     ...
     <main class="bg-light">
     	...
     	<h1>Manage your stocks</h1>
     	  <table class="table mt-4 w-100">
     	  	<c:choose>
     	  	<c:when test="${stocks.isEmpty()}">
     	  		<tr class="d-flex">
     	  			<td class="offset-4 col-4 text-danger">No stocks by your company!</td>
     	  		</tr>
     	  	</c:when>
     	  	<c:when test="${!stocks.isEmpty()}">
     	  		<c:forEach var="stock" items="${stocks}">
     	  		<c:choose>
     	  			<c:when test="${stock.getReserveDate() != null && LocalDateTime.now().isAfter(stock.getReserveDate()) }">
     					<tr class="d-flex">
     						<td class="offset-3 col-4">${stock.getStockName()}</td>
     						<td class="col-1"><a id="edit" href="/editStock?sId=${stock.getId()}" class="btn btn-info">Edit</a></td>
     						<td class="col-1"><a id="delete" href="/deleteStock?sId=${stock.getId()}" class="btn btn-danger">Delete</a></td>
     					</tr>
     				</c:when>
     				<c:when test="${stock.getReserveDate() != null && !LocalDateTime.now().isAfter(stock.getReserveDate()) }">
     					<tr class="d-flex">
     						<td class="offset-2 col-4">${stock.getStockName()}</td>
     						<td class="col-1"><a id="edit" href="/editStock?sId=${stock.getId()}" class="btn btn-info disabled">Edit</a></td>
     						<td class="col-1"><a id="delete" href="/deleteStock?sId=${stock.getId()}" class="btn btn-danger disabled">Delete</a></td>
     						<td class="text-danger">Stock is reserved!</td>
     					</tr>
     				</c:when>
     				<c:when test="${stock.getReserveDate() == null}">
     					<tr class="d-flex">
     						<td class="offset-3 col-4">${stock.getStockName()}</td>
     						<td class="col-1"><a id="edit" href="/editStock?sId=${stock.getId()}" class="btn btn-info">Edit</a></td>
     						<td class="col-1"><a id="delete" href="/deleteStock?sId=${stock.getId()}" class="btn btn-danger">Delete</a></td>
     		...
     		<tr class="d-flex">
     			<td class="offset-3 col-6"><a href="/epayCredentials">Provide ePay credentials</a></td>
     		</tr>
     	  </table>
     	 <h1 class="mt-4">Review your reserved stocks</h1>
     	 <a href="/resStocks" class="btn btn-info">Reserved Stocks</a>
     	 <h1 class="mt-4">Review your reservation history</h1>
     	 <a href="/resHistory" class="btn btn-info">Reservation History</a>
		</div>
     </main>
    <!-- Optional JavaScript -->
