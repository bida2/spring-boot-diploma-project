    ...
    <!-- Create navigation for the website within a <header> tag -->
     ...
     <main class="container text-center">
      <h4>Filter stock offers by Company Name</h4>
    	 <form method="POST" action="/stockBrowse/stockBrowseGroup">
                    <select class="form-control" name="groupName" class="mt-2">
                    	<c:forEach var="groupData" items="${groupList}">
                    	<c:choose>
                    		<c:when test="${param.groupName == groupData.stockGroupName}">
                    ...
                    <br>
					<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                    <input class="btn btn-primary mt-3" name="searchStockByGroup" type="submit" value="Submit"/>
          </form>  
          <div class="container mt-2">
          <c:if test="${not empty param.searchStockByGroup}">
